...
class TrainingManager(Entity):
    ...
    def input(self, key):
        ...
        if self.free_look:
            if key == 'scroll up':
                self.cam_dist = max(5, self.cam_dist - 2)
            if key == 'scroll down':
                self.cam_dist = min(50, self.cam_dist + 2)
    ...
    def update_adaptive_parameters(self):
        ...
        self.ui_info.text = f"Mut: {self.current_mutation_rate:.2f} | Walls: {self.current_wall_count}"

    def start_generation(self):
        self.update_adaptive_parameters()
        self.ui_status.text = "Status: OK"
        self.ui_status.color = color.green
        self.log_manager.log_generation_start(self.generation, 1.0) 
        self.cleanup()
    ...
    def update(self):
        ...
        if config.TEST_MODE:
            if self.time_scale > 0: 
                self.step_simulation(min(time.dt * self.time_scale, 0.1))
        else:
            step_dt = 0.04
    ...
    def step_simulation(self, dt):
        ...
        for i, (tagger, runner) in enumerate(self.agents):
            ...
            if tagger.stuck_timer > 2.0: 
                tagger.change_score(-20 * dt, "stuck_penalty")
            if runner.stuck_timer > 2.0: 
                runner.change_score(-20 * dt, "stuck_penalty")
            if dist < 6.0: 
                runner.change_score(-10 * dt, "proximity_penalty")
            if dist < 1.3: 
    ...
    def evolve(self):
        ...
        if current_max_score > self.best_historical_score: 
        ...
        merger_injected = False
        if os.path.exists(config.LOAD_FILE):
            try:
                with open(config.LOAD_FILE, 'rb') as f:
                    data = pickle.load(f)
                    if data.get('best_score', -9999) > self.best_historical_score + 10:
                        self.log_manager.log_status(f"Merger injected a better model! (Score: {data['best_score']:.0f})")
                        self.load_brains()
                        merger_injected = True
            except Exception as e:
                pass
                
        if not merger_injected:
        ...
        if abs(turn_bias) > config.ANTI_SPIN_THRESHOLD: 
            self.bad_gen_count += 1
        else: 
            self.bad_gen_count = 0
            
        if self.bad_gen_count >= config.MAX_BAD_GENS: 
        ...
        new_pop = []
        local_gen_size = max(1, config.GEN_SIZE // config.NUM_PROCESSES)
        keep_count = max(1, int(local_gen_size * 0.1))
        ...
        while len(new_pop) < local_gen_size:
            ...
            p_t.mutate(self.current_mutation_rate) 
            p_r.mutate(self.current_mutation_rate)
    ...
    def save_brains(self, t, r):
        ...
        try:
            ...
            if self.generation % config.BACKUP_INTERVAL == 0:
                with open(f"backup_gen_{self.generation}.pkl", 'wb') as f: 
                    pickle.dump(data, f)
            self.log_manager.log_status(f"Saved Best Brains (Gen {self.generation})")
        except Exception as e: 
            self.log_manager.log_error(f"Failed to save brains: {e}")
    ...
    def load_brains(self):
        if os.path.exists(config.LOAD_FILE): 
            try:
                with open(config.LOAD_FILE, 'rb') as f:
                    data = pickle.load(f)
                    if data['t_brain'].hidden_size != config.HIDDEN_LAYER_SIZE: 
                        return 
                ...
                self.log_manager.log_error(f"Load failed: {e}")
                self.generation = 1
        else: 
            self.log_manager.log_status(f"Could not find {config.LOAD_FILE}, starting fresh.")